...
func GetNodeV4(urls configuration.Url, accessToken string, swarmID, nexusID, nodeID string) (*NewNode, error) {
	var err error
	var response NewNode
	url := urls.ChUrl + "/v4/swarms/" + swarmID + "/nexuses/" + nexusID + "/nodes/" + nodeID
...
func UpdateNodeV4(urls configuration.Url, accessToken string, swarmID, nexusID, nodeID string, nodeBody UpdateNewNodeRequestBody) error {
	var err error
	url := urls.ChUrl + "/v4/swarms/" + swarmID + "/nexuses/" + nexusID + "/nodes/" + nodeID
...
func DeleteNodeV4(urls configuration.Url, accessToken string, swarmID, nexusID, nodeID, token string) error {
	var err error
	url := urls.ChUrl + "/v4/swarms/" + swarmID + "/nexuses/" + nexusID + "/nodes/" + nodeID + "?token=" + token
...
func ListNodesV4(urls configuration.Url, accessToken string, swarmID string, nexusID, sort string, filter string) (*GenericPaginatedResponse[*NewNode], error) {

	var err error
	var finalResponse GenericPaginatedResponse[*NewNode]

	url := urls.ChUrl + "/v4/swarms/" + swarmID + "/nexuses/" + nexusID + "/nodes" + "?sort_key=" + sort + "&q=" + url.QueryEscape(filter)
