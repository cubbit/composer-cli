...
func SignInOperator(cmd *cobra.Command, args []string) error {
	...
	utils.PrintSuccess(fmt.Sprintf("operator %s signed in successfully", email))

	return nil
}

func SignInAccount(cmd *cobra.Command, args []string) error {
	var err error
	var apiServerUrl, email, password, code, tenantID, refreshToken, profile, configPath string
	var challenge *api.ChallengeResponseModel
	var urls *configuration.Url
