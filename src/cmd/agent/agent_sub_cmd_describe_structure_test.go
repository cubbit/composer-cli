...
func TestAgentSubCmd_Structure_DescribeWithAllRequiredFlags(t *testing.T) {
	mockService := service.NewAgentServiceMock()

	mockService.DescribeAgentFunc = func(cmd *cobra.Command, args []string) error {
		cmd.Println("Mock: Agent described successfully")
		return nil
	}
	...
	expectedOutput := "Mock: Agent described successfully\n"
	if commandOutputString != expectedOutput {
		t.Fatalf("Expected output %q, got %q", expectedOutput, commandOutputString)
	}
}

func TestAgentSubCmd_Structure_DescribeWithMissingRequiredFlags(t *testing.T) {
	mockService := service.NewAgentServiceMock()
