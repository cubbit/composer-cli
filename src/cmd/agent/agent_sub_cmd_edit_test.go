...
func TestAgentSubCmd_Structure_EditWithAllRequiredFlags(t *testing.T) {
	mockService := service.NewAgentServiceMock()

	mockService.EditAgentFunc = func(cmd *cobra.Command, args []string) error {
	...
	expectedOutput := "Mock: Agent edited successfully\n"
	if commandOutputString != expectedOutput {
		t.Fatalf("Expected output %q, got %q", expectedOutput, commandOutputString)
	}
}

func TestAgentSubCmd_Structure_EditWithMissingFlags(t *testing.T) {
	mockService := service.NewAgentServiceMock()
