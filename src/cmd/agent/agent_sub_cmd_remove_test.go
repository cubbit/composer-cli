...
func TestAgentSubCmd_Structure_RemoveWithAllRequiredFlags(t *testing.T) {
	mockService := service.NewAgentServiceMock()

	mockService.RemoveAgentFunc = func(cmd *cobra.Command, args []string) error {
	...
	expectedOutput := "Mock: Agent removed successfully\n"
	if commandOutputString != expectedOutput {
		t.Fatalf("Expected output %q, got %q", expectedOutput, commandOutputString)
	}
}

func TestAgentSubCmd_Structure_RemoveWithMissingRequiredFlags(t *testing.T) {
	mockService := service.NewAgentServiceMock()
